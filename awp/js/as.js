    ...
    as.queryFun("queryActivityList", {
        ...
        if(res && res.length > 0){
            for(var i = 0;i < res.length;i++){
                res[i]["create_time"] = as.timestampFormat(res[i]["create_time"]);
...
as.queryActivity = function(activity_id, callback){
    as.queryFunV2("queryActivity", {
        ...
        if(res && res[0].length > 0){
            data = res[0][0];
            data.create_time = as.timestampFormat(data.create_time);
            if(data.img_url != ""){
                data.img_url = data.img_url.split(",");
            }else{
                data.img_url = [];
            }
        }
...
//获取相关案例
as.queryRelatedCases = function(activity_id, callback){
    as.queryFun("queryRelatedCases", {
        "activity_id" : activity_id
    }, function(res){
        if(res && res.length > 0){
            for(var i = 0;i < res.length;i++){
